...
export const googleAuth = async (req, res) => {
    try {
        ...
        // Step 1: Verify with Google — capture the return value
        let googleUser;
        try {
            if (accessToken) {
                googleUser = await verifyGoogleWebOwnership(accessToken); // ✅ Capture the return value
            }
            else if (idToken) {
                googleUser = await verifyGoogleOwnership(idToken); // ✅ Capture the return value
            }
        }
        catch (err) {
        ...
        // Now googleUser will have the user data
        ...
        if (user) {
            ...
            // ---- REGISTER PATH ----
            // domain_type is required for a brand-new account
            if (!domain_type) {
                return res.status(400).json({
                    success: false,
        ...
        if (isNewUser) {
            if (domain_type === 'company') {
                await new Company({ user_id: user._id, whatsapp_number: user.whatsapp_number }).save();
            } else if (domain_type === 'salon') {
                await new Salon({ user_id: user._id, whatsapp_number: user.whatsapp_number }).save();
...
export const resendOtp = async (req, res) => {
    try {
        ...
        // ✅ Save new OTP + UID
        if (otpResponse.uid) {
            user.whatsapp_uid = otpResponse.uid;  // 🔹 store provider UID
        }
    ...
    try {
        ...
        if (!user.otp_verified) {
            try {
                ...
                if (otpResponse.uid) {
                    user.whatsapp_uid = otpResponse.uid;
                }
                user.otp_code = otp;
                user.otp_expires_at = setOtpExpiry();
                user.otp_attempts = 0;
...
export const adminSendOtp = async (req, res) => {
    try {
        ...
        if (!["superadmin", "admin"].includes(user.domain_type)) {
...
// Admin OTP Login - Verify OTP & Issue Tokens
